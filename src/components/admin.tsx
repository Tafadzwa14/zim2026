      ...
      {open && (
        <div className="space-y-3 bg-chip/40 px-4 pb-4 pt-1">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!isSelf && <SmallBtn onClick={() => { if (confirm(u.claimed ? `Reset the PIN and revoke every session for ${u.name}?` : `Let ${u.name} set up their PIN?`)) run(() => actions.adminResetPin(u.id)); }}>{u.claimed ? "Reset PIN" : "Allow PIN setup"}</SmallBtn>}
...
// ---------------------------------------------------------------- nudge unclaimed
/** Lists people who haven't claimed their identity yet. */
    ...
    <div className="zc-card overflow-hidden p-0">
      {pending.map((u) => (
        <div key={u.id} className="flex items-center gap-3 border-b border-line2 px-4 py-3 last:border-0">
          ...
          <span className="text-right text-[11px] font-semibold text-muted">They can set up<br />their PIN now</span>
        </div>
      ))}
    </div>
  );
